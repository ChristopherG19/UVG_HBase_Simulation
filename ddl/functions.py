...
def dropTable(Hfiles, command):
    ...
    if(not exists(Hfiles, command)):
        return f"Table {TableName} not found, no changes"
    
    veri = checkStatus(Hfiles, command)
    
    if(not veri == False):
        return (f"{TableName} is not disabled, not changes", Hfiles)
    
    for region in Hfiles:
        for table in Hfiles[region]:
            if TableName == table:
                del Hfiles[region]
                return (f"{TableName} dropped", Hfiles)
        ...
        if res and (res.lower() == "y"):
            ...
            if res2 and (res2.lower() == "y"):
                ...
                for region, table in to_delete:
                    del Hfiles[region]
                return (f"{len(coincidences)} tables successfully dropped", Hfiles)
            else:
                print("Cancelando operacion")
                return ("No changes", Hfiles)
        else:
            print("Cancelando operacion")
            return ("No changes", Hfiles)
    else:
        return ("No hay coincidencias", Hfiles)
    
def describe(Hfiles, command):
    TableName = get_info(command)[0]
    if(not exists(Hfiles, command)):
        return (f"Table {TableName} not found", TableName, Hfiles)
